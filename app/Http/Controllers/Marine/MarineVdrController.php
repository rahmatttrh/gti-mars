...
class MarineVdrController extends Controller
{
   public function index()
   {
      ...
      $vdrs = Vdr::where('status', '>=', 1)->whereBetween('date', ['2025-09-16', $to])->orderBy('date', 'desc')->get();
      // dd($vdrs);
   ...
   public function historyList()
   {
      ...
      if (auth()->user()->hasRole('suptent_loc')) {
         ...
         $employee = Employee::where('username', auth()->user()->username)->first();
         if ($employee->area != null) {
            $vdrs = Vdr::where('area',  $employee->area)->whereIn('status', [3, 4])->orderBy('updated_at', 'desc')->paginate(800);
         } else {
            $vdrs = Vdr::where('func', $employee->func)->whereIn('status', [3, 4])->orderBy('updated_at', 'desc')->paginate(800);
         }
   ...
   public function approveSuptent($id)
   {
      ...
      return redirect()->back()->with('success', 'VDR Superintendent Approved');
   }
   ...
   public function reject(Request $req)
   {
      ...
      if (count($vdrRejectTables) > 0) {
         foreach ($vdrRejectTables as $reject) {
            $reject->delete();
         }
      }

      foreach ($req->data as $d) {
         // dd($d);
         VdrReject::create([
            'vdr_id' => $vdr->id,
            'table' => $d
   ...
   public function rejectFromEmailStore(Request $req)
   {
      ...
      if ($req->user == 'pet') {
         ...
         $status = 505;
         $user = User::where('username', 'suptent')->first();
      }
