...
class IntermilanController extends Controller
{
   ...
   public function selectScheduleListOld(Request $req)
   {
      ...
      $vessel = Vessel::find($schedule->vessel_id);
      $weight = $schedule->total_weight + $cargoItem->weight;
      $size = $schedule->total_size + $cargoItem->size;
      ...
      if ($scheduleRoutes->count() > 0) {
         // dd('ada schedule route');
         $lastScheduleRoutesA = ScheduleRoute::where('schedule_id', $schedule->id)->orderBy('created_at', 'desc')->first();
         $routeFrom = ScheduleRoute::where('schedule_id', $schedule->id)->where('port_id', $cargoItem->request->origin_id)->first();
         if (!$routeFrom) {
            // dd('from tidak ada di route');
            ScheduleRoute::create([
               'schedule_id' => $schedule->id,
               'request_id' => $cargoItem->request->id,
               'cargo_item_id' => $cargoItem->id,
               'date' =>  Carbon::now(),
               'port_id' => $cargoItem->request->origin_id,
               'rank' => $lastScheduleRoutesA->rank + 1,
               'date' => null,
               'status' => 1
            ]);
         ...
         $lastScheduleRoutes = ScheduleRoute::where('schedule_id', $schedule->id)->orderBy('created_at', 'desc')->first();
         $route = ScheduleRoute::where('schedule_id', $schedule->id)->where('port_id', $cargoItem->request->destination_id)->first();
      ...
      $schedule->update([
         'total_size' => $schedule->total_size + $cargoItem->size,
         'total_weight' => $schedule->total_weight + $cargoItem->weight
      ]);
